...
function prepare_tunnel_cfg() {
    ...
    ipaddress=$(ip address show ens3 | grep inet | tr -s ' ' | cut -d' ' -f3 | cut -d'/' -f1)
...
function main() {
    if [ -f /usr/sbin/ipsec-partner ]; then
       ...
       echo "ipsec-partner is not available in /usr/sbin. Use install function: ipsec-partner-install"
    fi
}
